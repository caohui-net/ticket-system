    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F7F4EF', padding: '2rem 1rem' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
          <button onClick={() => navigate('/tickets')} style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 400, color: '#5C635D', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', padding: '0.5rem' }}>← 返回列表</button>
          <div style={{ flex: 1 }}><h1 style={{ fontFamily: '"Fraunces", serif', fontSize: '2rem', fontWeight: 400, color: '#1F2421', margin: 0, letterSpacing: '-0.02em' }}>创建<span style={{ fontStyle: 'italic', color: '#C4612F' }}>工单</span></h1></div>
        </div>
        <form onSubmit={handleSubmit}>
          <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', padding: '2rem', border: '1px solid #E7E1D7', boxShadow: '0 1px 3px rgba(0, 0, 0, 0.05)' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end', paddingTop: '1.5rem', borderTop: '1px solid #E7E1D7' }}><button type="button" onClick={() => navigate('/tickets')} disabled={submitting} style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.9375rem', fontWeight: 400, color: '#5C635D', backgroundColor: '#FFFFFF', border: '1px solid #E7E1D7', borderRadius: '999px', padding: '0.75rem 1.75rem', cursor: submitting ? 'not-allowed' : 'pointer' }}>取消</button><button type="submit" disabled={submitting || !formData.title.trim() || !formData.description.trim()} style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.9375rem', fontWeight: 500, color: '#FFFFFF', backgroundColor: submitting || !formData.title.trim() || !formData.description.trim() ? '#E7E1D7' : '#C4612F', border: 'none', borderRadius: '999px', padding: '0.75rem 1.75rem', cursor: submitting || !formData.title.trim() || !formData.description.trim() ? 'not-allowed' : 'pointer', transition: 'all 0.2s ease' }}>{submitting ? '创建中...' : '创建工单'}</button></div>
          </div>
        </form>
      </div>
    </div>
